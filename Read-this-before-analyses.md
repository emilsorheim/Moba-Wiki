...
# Use of data
<!-- A guideline for use of MoBa data can be found on wiki page [User guides](User-guides) (only available in Norwegian). Below, some important points are listed. -->
## Generated variables
...
* **Data files based on data after birth** *(questionnaire at 6 months (Q4), 18 months (Q5), 3 years (Q6), 5 years (Q5Y), 7 years (Q7Y) etc.)*: Include an identifier for pregnancy (‘PREG_ID_XX’) and a identifier for the child (‘BARN_NR’). Both variables ‘PREG_ID_XX’ and ‘BARN_NR’ must be used when merging files based on data after birth to ensure that data is linked correctly for twins/triplets.
* **For the second Father questionnaire** *(QF2)*:
  * _**Data file based on data from Part 1 (about the father)**_ include an unique identifier for the father called ‘F_ID_XX’. This variable must be used as the key variable when merging files.
  * _**Data file based on data from Part 2 (about the child)**_ include an identifier for pregnancy (‘PREG_ID_XX’) and a identifier for the child (‘BARN_NR’). Both variables ‘PREG_ID_XX’ and ‘BARN_NR’ must be used when merging files based on data after birth to ensure that data is linked correctly for twins/triplets.

Syntax for merging files can be found on the wiki page [Merge files](Merge-files).
